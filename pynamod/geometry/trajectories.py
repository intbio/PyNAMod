...
class H5_Trajectory(Trajectory):
    def __init__(self,filename,initial_len,data_len,prot_len,mode='w',attrs_names=None,shapes=None,string_format_val=5,**kwards):
        if shapes:
            self.shapes = shapes
        else:
            self.shapes = [(data_len,1,3),(data_len,3,3),(data_len,6),(prot_len,1,3)]
...
class Integrator_Trajectory(Tensor_Trajectory):
    def __init__(self,proteins_list,dtype,traj_len,data_len):
        ...
        super().__init__(dtype,traj_len,data_len,torch.tensor,
                         shapes=[(traj_len,self.prot_origins_ln,1,3)],attrs_names=['prot_origins'])
