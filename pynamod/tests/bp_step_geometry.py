...
from pynamod.tests.geometry_transformations import *
...
def rebuild_by_ori_and_mat(ref_frames,ori):
    params = np.zeros((ori.shape[0],6))
    for i,(r,o) in enumerate(zip(ref_frames[1:],ori[1:])):
        params[i+1] = get_params_for_single_step_stock(o,ori[i],r,ref_frames[i])
