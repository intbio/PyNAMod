...
class Pairs_Storage(Structures_Storage):
    ...
    def sort(self):
        ...
        # Get leading_strand for each nucleotide in the pairs
        leading_strands = [self.nucleotides_storage[getattr(self, self.get_name('lead_nucl_ind'))[i]].leading_strand for i in range(len(self))]
        if len(leading_strands) == 0:
            return self
        if sum(leading_strands) == len(leading_strands):
            ...
            leading_result = self[leading_strands]
            lagging_result = self[[not i for i in leading_strands]]

            # Ensure both results are Pairs_Storage objects
            if not isinstance(leading_result, Pairs_Storage):
                new_leading = Pairs_Storage(self.structure_class, self.nucleotides_storage)
                new_leading.append(leading_result)
                leading_result = new_leading

            if not isinstance(lagging_result, Pairs_Storage):
                new_lagging = Pairs_Storage(self.structure_class, self.nucleotides_storage)
                new_lagging.append(lagging_result)
                lagging_result = new_lagging

            return leading_result + lagging_result
