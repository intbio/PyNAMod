...
class Iterator:
    ...
    def run(self, target_accepted_steps=int(1e5), max_steps=int(1e6), transfer_to_memory_every=None, save_every=1, rebuild_every=400,
        ...
        while self.total_step < max_steps and self.accepted_steps < target_accepted_steps:
            ...
            self._stats_display.show_step_data(self.accepted_steps, self.total_step, self.trajectory.energies.sum().item())
            #Stop only when a step is completed in case of keyboard interrupt.
            if self._stop_loop:
                break

        self._transfer_to_memory(steps=self.trajectory.cur_step)
        self._stats_display.show_final_data(target_accepted_steps, self._stop_loop,
                                            self.accepted_steps, self.total_step)
    ...
    def _prepare_system(self, target_accepted_steps, transfer_to_memory_every, rebuild_every,
        ...
        self.total_step = self.accepted_steps = 0
        if traj_init_step is None:
            traj_init_step = self.res_trajectory.cur_step
        ...
        self.trajectory.energies = torch.stack(self.energy.get_energy_components(self.trajectory))
        self.res_trajectory._set_frame_attr('energies', self.trajectory.energies.cpu())
    ...
    def _create_tens_trajectory(self, dtype):
        ...
        self.trajectory = Tensor_Trajectory(dtype,traj_len,ln,torch.tensor,attrs_names=['rlsp_origins','energies'],shapes=[init_rlsp_ori.shape,(4,)])
    ...
    def _integration_step(self, save_every, integration_mode):
        ...
        e_dif_components, e_mat, s_mat = self.energy.get_energy_dif(self._rotation_handler, change_indices[1], self.trajectory.energies)
        ...
        if self.debug:
            ...
            energy_error = ((cur_e - self.trajectory.energies) - e_dif_components)
        ...
        if not Del_E.isnan() and Del_E < 0 or (not (torch.isinf(torch.exp(Del_E))) and (r  <= torch.exp(-Del_E/self._scaled_KT))):
            ...
            self.trajectory.energies += e_dif_components
            ...
            if self.accepted_steps % self.rebuild_every == 0:
                ...
                self._rotation_handler.rlsp_origins = torch.vstack(rlsp_origins)
                self.trajectory.energies = torch.stack(self.energy.get_energy_components(self._rotation_handler))

            self._rotation_handler.set_new_traj_params(self.trajectory)

            if (self.accepted_steps % save_every) == 0:
                prev_e = self.trajectory.energies
                self.trajectory.cur_step += 1
                if self.trajectory.cur_step == self.transfer_to_memory_every:
                    self.trajectory.energies = torch.hstack(self.energy.get_energy_components(self._rotation_handler))
                    self._transfer_to_memory()

                    self.trajectory.cur_step = 0

                self._rotation_handler.set_new_traj_params(self.trajectory)
                self.trajectory.energies = prev_e
    ...
    def _transfer_to_memory(self, steps=None):
        if steps is None:
            steps = self.transfer_to_memory_every + 1
        origins_traj = self.trajectory.origins_traj[1:steps].numpy(force=True)
        ref_frames_traj = self.trajectory.ref_frames_traj[1:steps].numpy(force=True)
        local_params_traj = self.trajectory.local_params_traj[1:steps].numpy(force=True)
        rlsp_origins_traj = self.trajectory.rlsp_origins_traj[1:steps].numpy(force=True)
        energy_comp_traj = self.trajectory.energies_traj[1:steps].numpy(force=True)

        self.res_trajectory.add_frames(start_step=self.res_trajectory.cur_step+1, end_step=self.res_trajectory.cur_step+ steps ,origins_traj=origins_traj,
                                          ref_frames_traj=ref_frames_traj, local_params_traj=local_params_traj,
                                          rlsp_origins_traj=rlsp_origins_traj, energies_traj=energy_comp_traj)