...
class _Real_Space_Energy_Calculator():
    ...
    def update_matrices(self,e_mat,s_mat,rlsp_ind):
        old_s_mat = self._get_matr_slices(self.sp_en_mat,rlsp_ind)
        old_e_mat = self._get_matr_slices(self.es_en_mat,rlsp_ind)
        sl = old_s_mat == 0
        old_s_mat[:] = s_mat
        old_e_mat[:] = e_mat
        old_s_mat[sl] = 0
    ...
    def get_energy_dif(self,rlsp_origins,rlsp_change_index):

        radii_sum_prods = self._get_matr_slices(self.radii_sum_prod,rlsp_change_index)
        charges_multipl_prods = self._get_matr_slices(self.charges_multipl_prod,rlsp_change_index)
    ...
    def _get_real_space_total_energy(self,origins,rlsp_origins,save_matr=True):
        ...
        dist_matrix = self._cdist(rlsp_origins,rlsp_origins)
        ...
        if save_matr:
            self.es_en_mat = torch.zeros(rlsp_origins.shape[0],rlsp_origins.shape[0],device=e_mat.device,dtype=e_mat.dtype)
